...
#shanghai
# Mean: [0.40891386 0.42173112 0.35373527]
# Std: [0.17121046 0.14773424 0.14963165]
...
def train(train_loader, model, criterion, optimizer, epoch, opt):
    ...
    for idx, (images, images_cp, images_human, images_geo) in enumerate(train_loader):
        data_time.update(time.time() - end)

        # images = torch.cat([images[0], images[1]], dim=0)
        if torch.cuda.is_available():
            images = images.cuda(non_blocking=True)
            images_cp = images_cp.cuda(non_blocking=True)
            images_human = images_human.cuda(non_blocking=True)
            images_geo = images_geo.cuda(non_blocking=True)
        bsz = images.shape[0]
        ...
        features2 =model(images_cp)
        features3 = model(images_human)
        features4 = model(images_geo)
        f1, f2, f3, f4 = features.reshape(bsz,1,-1), features2.reshape(bsz,1,-1), features3.reshape(bsz,1,-1), features4.reshape(bsz,1,-1)
        # f1, f2 = torch.split(features, [bsz, bsz], dim=0)

        if opt.method == 'SupCon':
            loss = criterion(f1, f2, f3, f4)
...
def main():
    ...
    torch.save(model.state_dict(), './save/final_dict_eshanghai.pth')
